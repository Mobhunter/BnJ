...
let get_info_url = new URL(window.location.origin + "/api/account/get_info");

let userid = $.urlParam("userid");
if (userid != null)
	get_info_url.searchParams.append("userid", userid);

$.ajax({ 
	url: get_info_url.href, 
    dataType: "json", 
	method: "GET", 
...
var create_post_api = new URL(window.location.origin + "/api/account/create_post");
$.ajax({
	statusCode: {
		404: function() {
	...
	headers: {'X-CSRFToken': csrftoken},
	url: create_post_api.href, 
	dataType: "json", // Для использования JSON формата получаемых данных
	contentType: "application/json",
	method: "POST", // Что бы воспользоваться POST методом, меняем данную строку на POST   	
	data: JSON.stringify({message: post_storage.message, audio: post_storage.audio})
	}).done(function(data) {
		return
	});

/*мозг артёма выходит в окно*/
